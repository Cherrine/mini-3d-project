function init() {
    ...
    var plane = getPlane(planeMaterial,15);
    var sphereGrid = getSphereGrid(10, 1.5);
...
function getSphereGrid(amount, separationMultiplier) {
    var group = new THREE.Group(); // Group for combining multiple objects
    for (var i = 0; i < amount; i++) {
        for (var j = 0; j < amount; j++) {
            var sphereMaterial = getMaterial('rgb(255, 245, 245)');
            var obj = getSphere(sphereMaterial, 0.5, 24); // Create a sphere with size 0.5
